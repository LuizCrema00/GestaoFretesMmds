<x-layout tittle="Controle de Clientes">
            ...
            <div class="btn-group btn-group-lg">
                ...
                <div class="dropdown">
                    <button class="btn btn-primary dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        ...
                        </svg>    Controle de Caminhões
                    </button>
                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                        <a class="dropdown-item" href="{{ route('caminhao-create')}}">Cadastrar caminhão</a>
                        <a class="dropdown-item" href="{{ route('caminhao-index')}}">Consultar caminhão</a>
                    </div>
                </div>
                <div class="dropdown">
                    <button class="btn btn-primary dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
    ...
    <br>
    <div>
        <a href="{{ route('cliente-create') }}"  style="margin-inline: 220px" class="btn btn-primary me-2">
    ...
    <table class="table datatables">
        ...
        <tr>
            <th>{{ $cliente->nome }}</th>
            <th class="text-end">{{ $cliente->CNPJ }}</th>
            <th class="text-end">{{ $cliente->CEP }}</th>
            <th>{{ $cliente->UF }}</th>
            <th>{{ $cliente->Cidade }}</th>
            <th>{{ $cliente->Logradouro }}</th>
            <th class="text-end">{{ $cliente->Numero }}</th>
            <th>{{ $cliente->Bairro }}</th>
            <th class="text-end">{{ $cliente->Fone }}</th>
            <th class="d-flex">
                <a href="{{ route('cliente-edit', ['id'=>$cliente->id]) }}" class="btn btn-primary me-2">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil" viewBox="0 0 16 16">
                        <path d="M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168l10-10zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207 11.207 2.5zm1.586 3L10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293l6.5-6.5zm-9.761 5.175-.106.106-1.528 3.821 3.821-1.528.106-.106A.5.5 0 0 1 5 12.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.468-.325z"/>
                    </svg>
                </a>
                <a class="btn btn-danger" href="#" data-toggle="modal" data-target="#ModalDelete{{$cliente->id}}">
                ...
                <form action="{{route('cliente-destroy', $cliente->id) }}" method="post" enctype="multipart/form-data">
                    {{ method_field('delete') }}
                    {{ csrf_field() }}
                    <div class="modal fade" id="ModalDelete{{$cliente->id}}" tabindex="-1" role="dialog" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                ...
                                <div class="modal-body"> Voce tem certeza que deseja excluir {{$cliente->nome}}? Este cliente possui ou pode possuir Fretes que também serão excluídos.</div>
